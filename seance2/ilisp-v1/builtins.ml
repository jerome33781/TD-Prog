...
let eval_add arg1 arg2 =
  let rec add a b =
    match a with
      Atom s1 ->
      begin match b with
	  Atom s2 -> Atom ( string_of_int ( (int_of_string s1) + (int_of_string s2)) )
	| Cons(car2,cdr2) -> Atom ( string_of_int ( (int_of_string s1) + ( int_of_string (add car2 cdr2)) ) )
      end
    | Cons(car1,cdr1) ->
       begin match b with
	  Atom s2 ->  Atom ( string_of_int ( (int_of_string s2) + ( int_of_string (add car1 cdr1)) ) )
	| Cons (car2,cdr2) -> add ( add car1 car2 ) ( add cdr1 cdr2)
      end
...
let eval_add arg =
  let rec add arg =
    match arg with
      | Atom "nil" -> Atom "0"
      | Atom s1 -> Atom  s1
      | Cons(car,cdr) -> Atom (  string_of_int ( ( (int_of_atom ( add  car )) )  +  (int_of_atom (  add cdr )) ) )
  in add arg
...
let eval_sub arg =
  let rec sub arg =
    match arg with
      | Atom "nil" -> Atom "0"
      | Atom s1 -> Atom  s1
      | Cons(car,cdr) -> Atom (  string_of_int ( ( (int_of_atom ( sub  car )) )  -  (int_of_atom (  sub cdr )) ) )
  in sub arg
...
let eval_mul arg =
  let rec mul arg =
    match arg with
      | Atom "nil" -> Atom "1"
      | Atom s1 -> Atom  s1
      | Cons(car,cdr) -> Atom (  string_of_int ( ( (int_of_atom ( mul  car )) )  *  (int_of_atom (  mul cdr )) ) )
  in mul arg

(* interpret ( greater arg1 arg2 ) *)
let eval_greater arg1 arg2 =
  let rec greater a b =
    match a with
      Atom s1 ->
      begin match b with
	  Atom s2 ->  Atom ( string_of_bool ( (int_of_string s1) > (int_of_string s2)) )
	| Cons _ -> raise (Error "could not compare arguments")
      end
    | Cons _ -> raise (Error "could not compare arguments")
  in greater arg1 arg2

(* interpret ( lesser arg1 arg2 ) *)
let eval_lesser arg1 arg2 =
  let rec lesser a b =
    match a with
      Atom s1 ->
      begin match b with
...
let all_builtins =
  [ (* Special forms *)
    ...
    "greater", BuiltinFn2 eval_greater;
    "lesser", BuiltinFn2 eval_lesser;
